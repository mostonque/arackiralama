...
class aracEkleController extends adminBaseController{
    ...
    public function aracEkle(){
        ...
        if(isset($_POST['motorGucu']) && !empty($_POST['motorGucu'])  && is_numeric($_POST['motorGucu'])&& strlen($_POST['motorGucu'])<=6)
        {
            $motorGucu=htmlspecialchars(addslashes(trim($_POST['motorGucu'])));
        }else{
            $hata="Motor Gücü alanı hatalı.Kontrol ediniz";
        }
        if(isset($_POST['motorHacmi']) && !empty($_POST['motorHacmi']) && is_numeric($_POST['motorHacmi']) && strlen($_POST['motorHacmi'])<=6)
        {
